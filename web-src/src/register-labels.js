const CN_TO_EN = {
  ...
  水箱模式制热模式设定温上限: 'tank_and_heating_set_temperature_upper_limit',
...
const EN_TO_CN = Object.fromEntries(
  Object.entries(CN_TO_EN).map(([cn, en]) => [en, cn])
)

const EN_ACRONYMS = new Set(['ac', 'ct', 'hp', 'eev', 'modbus'])

function toEnglishTitleCase(snake) {
  return String(snake)
    .split('_')
    .filter((x) => x)
    .map((token) => {
      const lower = token.toLowerCase()
      if (EN_ACRONYMS.has(lower)) {
...
export function lookupRegisterLabel(name) {
  const raw = String(name || '')
  if (Object.prototype.hasOwnProperty.call(CN_TO_EN, raw)) {
    return { en: CN_TO_EN[raw], cn: raw }
  }
  if (Object.prototype.hasOwnProperty.call(EN_TO_CN, raw)) {
    return { en: raw, cn: EN_TO_CN[raw] }
  }
  return { en: raw, cn: raw }
}

export function displayRegisterName(name, lang) {
  const pair = lookupRegisterLabel(name)
  if (lang === 'zh') {
    return pair.cn
  }
  return toEnglishTitleCase(pair.en)
}
